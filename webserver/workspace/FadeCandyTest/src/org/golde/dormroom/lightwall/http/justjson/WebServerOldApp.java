...
public class WebServerOldApp {

    private SceneHandler animationHandler;
    public static InetSocketAddress IP_AND_PORT;

    public WebServerOldApp(SceneHandler animationHandler) {
        this.animationHandler = animationHandler;
    }

    public void start(String ip, int port) {
        try {
            IP_AND_PORT = new InetSocketAddress(ip, port); //get correct 1.9 or 10. address and not 0.0.0.0
            HttpServer server = HttpServer.create(IP_AND_PORT, 0);
            server.createContext("/", new MyHandler());
            server.setExecutor(null); // creates a default executor
            server.start();
            System.out.println("OLD Router started on: " + ip + ":" + port);
        }
        catch(Exception e){
            e.printStackTrace();
        }
